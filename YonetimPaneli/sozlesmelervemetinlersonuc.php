...
if(isset($_SESSION["Yonetici"])){
    ...
    if(isset($_POST["UyelikSozlesmeMetni"])){
        $GelenUyelikSozlesmesiMetni			=	Guvenlik($_POST["UyelikSozlesmeMetni"]);
    }else{
    ...
    if(($GelenHakkimizdaMetni!="") and ($GelenUyelikSozlesmesiMetni!="") and ($GelenKullanimKosullariMetni!="") and ($GelenGizlilikSozlesmesiMetni!="") and ($GelenMesafeliSatisSozlesmesiMetni!="") and ($GelenTeslimatMetni!="")){
        $MetinleriGuncelle			=	$VeritabaniBaglantisi->prepare("UPDATE sozlesmelervemetinler SET HakkimizdaMetni = ?, UyelikSozlesmeMetni = ?, KullanimKosullariMetni = ?, GizlilikSozlesmesiMetni = ?, MesafeliSatisSozlesmesiMetni = ?, TeslimatMetni = ?, IptalIadeDegisimMetni = ?");
